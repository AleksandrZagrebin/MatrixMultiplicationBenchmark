...
После сборки проекта появляется 3 исполняемых файла.
...
| Файл | Описание |
|------|----------|
| `./squarebench` | Бенчмарк для **квадратных** матриц. Размеры заданы в коде: 100, 500, 1000, 1500. Результат — файл benchmark_results.txt. |
| `./plotter` | Построение графика по данным из `benchmark_results.txt`, полученным после запуска `squarebench`. |
| `./bench` | Бенчмарк для **произвольных** матриц. Размеры задаются через аргументы командной строки: `./bench AX AY BY` |

---

## Результаты
График сделан при помощи gnuplot и библиотеки sciplot.
